...
// register creativity_architect_Featured_Post widget
function creativity_architect_register_featured_post_widget() {
    register_widget( 'creativity_architect_Featured_Post' );
}
add_action( 'widgets_init', 'creativity_architect_register_featured_post_widget' );
 
 /**
 * Adds creativity_architect_Featured_Post widget.
 */
class creativity_architect_Featured_Post extends WP_Widget {
	...
	function __construct() {
		parent::__construct(
			'creativity_architect_featured_post', // Base ID
			__( 'RARA: Featured Post', 'the-creativity-architect' ), // Name
			array( 'description' => __( 'A Featured Post Widget', 'the-creativity-architect' ), ) // Args
		);
	}
            ...
            ?>
                ...
                <div class="img-holder">
                    <a href="<?php the_permalink(); ?>">
                        <?php the_post_thumbnail( 'creativity-architect-featured-post' ); ?>
                    </a>
                </div>    				
                <?php } ?>
	...
	public function form( $instance ) {
		$postlist[0] = array(
    		'value' => 0,
    		'label' => __('--Choose--', 'the-creativity-architect'),
    	);
        ...
        $read_more      = !empty( $instance['readmore'] ) ? $instance['readmore'] : __( 'Read More', 'the-creativity-architect' );	      
        $show_thumbnail = !empty( $instance['show_thumbnail'] ) ? $instance['show_thumbnail'] : '' ;        
        $post_list      = !empty( $instance['post_list'] ) ? $instance['post_list'] : 1 ;
        ?>
		<p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'post_list' ) ); ?>"><?php esc_html_e( 'Posts', 'the-creativity-architect' ); ?></label>
        ...
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'readmore' ) ); ?>"><?php esc_html_e( 'Read More Text', 'the-creativity-architect' ); ?></label> 
	...
	public function update( $new_instance, $old_instance ) {
        ...
        $instance['readmore']       = !empty( $new_instance['readmore'] ) ? sanitize_text_field( $new_instance['readmore'] ) : __( 'Read More', 'the-creativity-architect' );
...
} // class creativity_architect_Featured_Post